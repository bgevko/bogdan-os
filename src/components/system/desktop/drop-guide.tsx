...
const DropGuide = (): ReactElement | null => {
  ...
  const positions = useMemo(() => {
    ...
    if (
      ...
      const { x, y } = indexToPosition(guideIndex, getLineSize('/Desktop'));
      return [{ x: x * 100 + 24, y: y * 100 + 24 }];
    }

    // Normal case, dragging one or multiple files
    const newPositions = offsets.map((offset) => {
      const { x, y } = indexToPosition(guideIndex + offset, getLineSize('/Desktop'));
      return { x: x * 100 + 24, y: y * 100 + 24 };
    });
    return newPositions;
  }, [dragContext, dragoverPath, getLineSize, guideIndex, isDir, offsets, getGridIndex]);
