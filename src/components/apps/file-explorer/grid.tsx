...
const Grid = ({ children, path }: GridProps): ReactElement => {
  ...
  const handleDropOnFolderGrid = useCallback(
    (event: React.DragEvent) => {
      // Drop logic for dropping items on a folder grid
      ...
      for (const element of elementPaths) {
        ...
        // Move the file within the same grid
        if (element.path === destinationPath) {
          setGridIndex(element.path, finalIndex);
          // Otherwise, move it to a different grid
        } else {
          try {
    ...
    [setGridIndex, mv, setIsDragging, grid.lineSize, getWindow, path],
  );
